...
export class PerformanceMonitorComponent {
  ...
  private performanceTicker = signal(0);
  ...
  private setupPerformanceTracking() {
    ...
    effect(() => {
      ...
      // Read ticker to keep effect active and performanceMetrics for measurement
      this.performanceTicker();
      this.performanceMetrics();

      const end = performance.now();
      const renderTime = end - start;

      this.renderTimes.update(times => {
        const newTimes = [...times, renderTime];
  ...
  private estimateMemoryUsage(): number {
    ...
    const simulationUsage = this.simulationResults().length;

    return baseUsage + metricsUsage + simulationUsage + Math.random() * 10;
  }

  private startPerformanceTicker() {
    // Update ticker every 100ms to keep performance tracking active
    setInterval(() => {
      this.performanceTicker.update(tick => tick + 1);
    }, 100);
  }
}
